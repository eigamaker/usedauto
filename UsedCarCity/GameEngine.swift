...
final class GameEngine: ObservableObject {
    ...
    @Published var onlineListings: [OnlineListing] = []
    @Published var onlineBidReservations: [OnlineBidReservation] = []
    @Published var onlineBidResults: [OnlineBidResult] = []
    ...
    private struct SaveData: Codable {
        ...
        let onlineListings: [OnlineListing]
        let onlineBidReservations: [OnlineBidReservation]
        let onlineBidResults: [OnlineBidResult]
    ...
    private func apply(_ saved: SaveData) {
        ...
        onlineListings = saved.onlineListings
        onlineBidReservations = saved.onlineBidReservations
        onlineBidResults = saved.onlineBidResults
    ...
    func catalogMarketIndex(for model: VehicleCatalogEntry, in kind: DistrictKind) -> Double {
        ...
        case .imported, .suv, .sports: economyEffect = 0.72 + economicIndex * 0.28
    ...
    private func specialtyPriceCeiling(
        ...
        if let productState {
            switch productState {
            case .sportStreet: return 1.45
            case .sportDrift: return 1.75
            case .sportCircuit: return 2.00
            case .welfareLiftSeat: return 1.30
            case .welfareWheelchair: return 1.60
            case .mobileSales: return 1.45
            case .kitchenCar: return 1.90
            default: break
            }
        }
    ...
    private func segmentWillingnessFactor(
        ...
        let trendLift = max(0, activeTrendMultiplier(for: key) - 1)
        let normalizedTrend = min(1, trendLift / 2.2)
        let readiness = store.map { specialtyReadiness(for: $0, productKind: key.productKind) } ?? 0.65
        let ceiling = specialtyPriceCeiling(for: key.productKind, productState: productState)
    ...
    func createProcurementInstruction(
        ...
        guard stores.contains(where: { $0.id == storeID }),
              totalBudget > 0,
              financialRule.amount >= 0 else { return nil }
    ...
    func updateProcurementInstruction(_ changed: ProcurementInstruction) -> Bool {
        guard let index = procurementInstructions.firstIndex(where: { $0.id == changed.id }),
              ...
              changed.financialRule.amount >= 0 else { return false }
    ...
    private func cancelAutomaticReservations(instructionID: UUID) {
        ...
        onlineBidReservations.removeAll { $0.instructionID == instructionID }
    }
    ...
    @discardableResult
    func reserveBid(listingID: UUID, storeID: UUID, maxPrice: Int, instructionID: UUID? = nil) -> Bool {
        ...
        if let index = bidReservations.firstIndex(where: { $0.listingID == listingID }) {
            ...
            bidReservations[index].committedCost = maxPrice + listing.venue.fee + listing.venue.shippingCost
        } else {
            bidReservations.append(BidReservation(
                ...
                storeID: storeID,
                maxPrice: maxPrice,
                committedCost: maxPrice + listing.venue.fee + listing.venue.shippingCost,
                resultTurn: turn + 1,
    ...
    func onlineBidWinChance(for listing: OnlineListing, maxPrice: Int) -> Double {
        let sampleCount = 120
        var wins = 0
        let baseSeed = turn * 311 + listing.modelYear * 17 + listing.mileage / 500 + categoryIndex(listing.category) * 61
        for sample in 0..<sampleCount {
            let rival = competitorOnlineBid(for: listing, seed: baseSeed + sample * 7_919)
            if maxPrice >= (rival?.maxPrice ?? listing.reservePrice) { wins += 1 }
        }
        return Double(wins) / Double(sampleCount)
    }

    func onlineBidStep(for listing: OnlineListing) -> Int {
        let rawStep = max(5, listing.marketPrice / 40)
        return max(5, Int((Double(rawStep) / 5).rounded()) * 5)
    }

    @discardableResult
    func reserveOnlineBid(
        listingID: UUID,
        storeID: UUID,
        maxPrice: Int,
        instructionID: UUID? = nil
    ) -> Bool {
        guard let listing = onlineListings.first(where: { $0.id == listingID }),
        ...
        let otherReservedSlots = onlineBidReservations.filter { $0.storeID == storeID && $0.listingID != listingID }.count
            + bidReservations.filter { $0.storeID == storeID }.count
        guard store.inventoryCount + incomingCount(for: storeID) + otherReservedSlots + 1 <= store.type.capacity else { return false }
        if let index = onlineBidReservations.firstIndex(where: { $0.listingID == listingID }) {
            guard onlineBidReservations[index].instructionID == instructionID else { return false }
            onlineBidReservations[index].maxPrice = maxPrice
            onlineBidReservations[index].committedCost = maxPrice + listing.fee + listing.shippingCost
        } else {
            onlineBidReservations.append(OnlineBidReservation(
                id: UUID(),
                listingID: listingID,
                storeID: storeID,
                maxPrice: maxPrice,
    ...
    func cancelOnlineBid(listingID: UUID) {
        onlineBidReservations.removeAll { $0.listingID == listingID && $0.instructionID == nil }
        save()
    }
    ...
    func dealerTradeQuote(
        ...
        if let modelID,
           let selected = VehicleCatalog.entry(id: modelID),
           selected.category == category,
           selected.usedMarketTurn <= turn {
            model = selected
        } else {
            model = vehicleModel(for: category, seed: turn * 193 + categoryIndex(category) * 47)
        }
    ...
    func onlineAssessment(for listing: OnlineListing, storeID: UUID) -> VehicleAssessment {
        vehicleAssessment(
            source: .online,
    ...
    func purchaseExpectedGrossProfit(for item: PurchaseCase) -> Int {
        ...
        if let project = item.suggestedProjectKind,
           let store = stores.first(where: { $0.id == item.storeID }) {
            ...
            let staffDiscount = canDoInHouse ? 30 : 0
            let facilityDiscount = canDoInHouse ? 30 : 0
            let finalRate = max(40, 100 - staffDiscount - facilityDiscount)
    ...
    func tradeInRepairCost(for tradeIn: TradeInVehicle, storeID: UUID) -> Int {
        ...
        let staffDiscount = hasServiceTechnician(storeID: storeID) ? 30 : 0
        let facilityDiscount = store.serviceBays > 0 ? 30 : 0
        return max(1, tradeIn.repairCost * (100 - staffDiscount - facilityDiscount) / 100)
    }
    ...
    func acceptCustomizationOrder(_ orderID: UUID) -> Bool {
        ...
        let activeOrderBays = customerCustomizationOrders.filter {
            $0.storeID == order.storeID && $0.status == .active
        }.count
        guard stores[storeIndex].facilities.contains(.customWorkshop),
              stores[storeIndex].employees.contains(where: { $0.assignment == .service }),
              activeInventoryBays + activeOrderBays < stores[storeIndex].customizationBays,
    ...
    func saleNegotiationPreview(storeID: UUID, buyerLeadID: UUID, inventoryID: UUID, strategy: SaleNegotiationStrategy) -> (price: Int, grossProfit: Int, closeChance: Double)? {
        ...
        let rawOffer = Int(Double(quote.price) * purposeValue
            * segmentWillingnessFactor(for: desiredSegment, store: store, productState: batch.productState)
        ...
        if let model = VehicleCatalog.entry(id: batch.modelID),
           [.sportStreet, .sportDrift, .sportCircuit,
            .welfareLiftSeat, .welfareWheelchair,
            .mobileSales, .kitchenCar].contains(batch.productState) {
            let cap = Double(model.referenceRetailPrice)
                * specialtyPriceCeiling(for: marketProductKind(for: batch), productState: batch.productState)
                * (1 - campaignDiscount) * (1 - strategy.discountRate)
        ...
        let facilityEffect = facilityCloseAdjustment(store: store, category: batch.category, purpose: lead.purpose)
        ...
        let baselineChance = min(0.97, max(0.03, strategy.baseCloseChance + campaignCloseBonus + demandEffect + reputationEffect + preferenceEffect + catalogEffect + qualityEffect + yearEffect + mileageEffect + budgetEffect + specialtyEffect + disclosedIssueEffect + facilityEffect + competitiveEffect + directRivalEffect + expertiseEffect))
    ...
    private func recordBuyerReview(
        ...
        fairPrice = Int(Double(fairPrice)
            ...
            * segmentWillingnessFactor(for: fairKey, store: stores[storeIndex], productState: batch.productState))
    ...
    func workshopProjectPreview(
        ...
        if kind == .wheelchairConversion && ![VehicleCategory.kei, .minivan, .commercial].contains(batch.category) { return nil }
        if kind == .mobileSalesConversion && ![VehicleCategory.kei, .minivan, .commercial].contains(batch.category) { return nil }
        if kind == .kitchenCarConversion && ![VehicleCategory.minivan, .commercial].contains(batch.category) { return nil }
        ...
        let uncappedProjectedPrice = max(25, Int(Double(marketValue) * store.priceIndex * inventoryAgingValueFactor(for: projected) * specialtyMarketFactor(for: projected, in: plot.district) * purposeValue * disclosedIssueFactor))
        let specialtyPriceCap: Int? = switch targetState {
        case .camper: Int(Double(model.referenceRetailPrice) * 2.0)
        case .sportStreet: Int(Double(model.referenceRetailPrice) * 1.45)
        case .sportDrift: Int(Double(model.referenceRetailPrice) * 1.75)
        case .sportCircuit: Int(Double(model.referenceRetailPrice) * 2.0)
        case .welfareLiftSeat: Int(Double(model.referenceRetailPrice) * 1.30)
        case .welfareWheelchair: Int(Double(model.referenceRetailPrice) * 1.60)
        case .mobileSales: Int(Double(model.referenceRetailPrice) * 1.45)
        case .kitchenCar: Int(Double(model.referenceRetailPrice) * 1.90)
        default: nil
        }
    ...
    func advanceWeek() {
        ...
        resolveOnlineBids(at: turn + 1, notes: &notes)
        ...
        let automaticallyShowReport = persistenceEnabled
            ...
            : false
        showMonthlyReport = automaticallyShowReport && reportWeek == 4
        showWeeklyReport = automaticallyShowReport && reportWeek != 4
    ...
    func specialtyBrandProfile(for store: Store, productKind: MarketProductKind) -> SpecialtyBrandProfile {
        ...
        let currentTrend = segmentTrends
            .filter { $0.kind.productKind == productKind }
    ...
    private func facilityBuyerFactor(_ store: Store, category: VehicleCategory?, purpose: CustomerPurpose?) -> Double {
        var factor = 1.0
    ...
    func buyerAttractionFactor(for store: Store, category: VehicleCategory?, purpose: CustomerPurpose? = nil) -> Double {
        ...
        return policyBuyerFactor(store.marketPolicy, category: category)
            * facilityBuyerFactor(store, category: category, purpose: purpose) * expertise
    }
    ...
    private func facilityCloseAdjustment(store: Store, category: VehicleCategory, purpose: CustomerPurpose? = nil) -> Double {
        var adjustment = 0.0
        if store.facilities.contains(.kidsSpace), purpose == .family {
            adjustment += 0.08
        }
    ...
    func facilitySalesCloseBonus(
        ...
        facilityCloseAdjustment(store: store, category: category, purpose: purpose)
    }
    ...
    private func trendConfiguration(
        ...
        case .welfareDemand:
            return (Set(DistrictKind.allCases), [.kei, .compact, .minivan, .commercial])
        case .mobileBusinessBoom:
            return ([.downtown, .station, .industrial, .highway], [.kei, .minivan, .commercial])
        }
    }
    ...
    private func updateSegmentTrends(at resolvingTurn: Int, notes: inout [String]) {
        ...
        let peakMultiplier = peakRange.lowerBound
            + transactionRoll(seed: simulationSeed &+ resolvingTurn &* 8_261 + 59)
    ...
    private func progressWorkshopProjects(notes: inout [String]) {
        for storeIndex in stores.indices {
            ...
            for orderIndex in orderIndices {
                ...
                notes.append("\(stores[storeIndex].name)：持ち込み\(order.vehicleName)の\(order.kind.name)が完成（売上\(order.quotedRevenue.currency)）")
            }
    ...
    private enum AutomaticProcurementCandidateKind {
        ...
        case dealerTrade(quote: ProcurementQuote)
        case auction(listingID: UUID, maxPrice: Int)
        case online(listingID: UUID, maxPrice: Int)
    }
    ...
    private func estimatedSourcingRepairCost(
        ...
        let staffDiscount = store.employees.contains(where: { $0.assignment == .service }) ? 30 : 0
        let facilityDiscount = store.serviceBays > 0 ? 30 : 0
        return max(1, Int((Double(baseline) * Double(100 - staffDiscount - facilityDiscount) / 100).rounded()))
    }
    ...
    private func automaticProcurementCashReserved() -> Int {
        ...
        let online = onlineBidReservations
            .filter { $0.instructionID != nil }
            .reduce(0) { $0 + $1.committedCost }
    ...
    private func automaticProcurementCandidate(
        ...
        let visibility = min(0.95, max(0.55, 0.55 + handler.procurementComposite * 0.004))
        ...
        if instruction.allowedSources.contains(.dealerTrade), !instruction.faultOnly {
            ...
            for category in categories {
                ...
                guard let quote = dealerTradeQuote(
                    category: category,
                    count: 1,
                    storeID: storeID,
                    modelID: modelID
                ), quote.modelID == modelID || modelID == nil else { continue }
                let model = quote.modelID.flatMap(VehicleCatalog.entry(id:))
                ...
                let condition = VehicleConditionProfile(
                    exterior: Int(quote.quality * 100),
                    interior: Int(quote.quality * 100),
                    mechanical: Int(quote.quality * 100)
                )
                let predictedRetail = predictedSourcingRetail(
                    storeID: storeID,
                ...
                let maxOffer = maximumInstructionOffer(
                    ...
                    predictedRetail: predictedRetail,
                    repairCost: 0,
                    fixedCosts: quote.fee
                )
                guard quote.unitCost <= maxOffer,
                      quote.totalCost <= instruction.remainingBudget,
                      quote.totalCost <= cash else { continue }
                candidates.append(AutomaticProcurementCandidate(
                    kind: .dealerTrade(quote: quote),
                    source: .dealerTrade,
                    vehicleName: quote.vehicleName,
                    acquisitionCost: quote.totalCost,
                    predictedUnitGrossProfit: predictedRetail - quote.totalCost
                ))
            }
        }
        ...
        for listing in auctionListings where !bidReservations.contains(where: { $0.listingID == listing.id }) {
            ...
            let fixed = listing.venue.fee + listing.venue.shippingCost
            let predictedRetail = predictedSourcingRetail(
                storeID: storeID,
        ...
        if instruction.allowedSources.contains(.online) {
        for listing in onlineListings where !onlineBidReservations.contains(where: { $0.listingID == listing.id }) {
            ...
            guard maxPrice >= listing.reservePrice,
                  ...
                  automaticProcurementCashReserved() + committed <= cash else { continue }
            candidates.append(AutomaticProcurementCandidate(
                kind: .online(listingID: listing.id, maxPrice: maxPrice),
                source: .online,
                vehicleName: listing.vehicleName,
                acquisitionCost: committed,
                predictedUnitGrossProfit: predictedRetail - committed - repair
        ...
        return candidates.max {
            if $0.predictedUnitGrossProfit != $1.predictedUnitGrossProfit {
                return $0.predictedUnitGrossProfit < $1.predictedUnitGrossProfit
            }
    ...
    private func executeAutomaticProcurementCandidate(
        ...
        case .dealerTrade(let quote):
            guard quote.totalCost <= procurementInstructions[instructionIndex].remainingBudget,
                  cash >= quote.totalCost else { return false }
            cash -= quote.totalCost
            inboundShipments.append(InboundShipment(
                ...
                count: quote.count,
                unitCost: quote.unitCost + quote.fee / quote.count,
                quality: quote.quality,
                modelYear: nil,
                mileage: nil,
                instructionID: instructionID,
                acquiredTurn: turn,
                monthsRemaining: quote.weeks
            ))
            simulationTransactionHandler?(SimulationVehicleTransaction(
                turn: turn,
                kind: .acquired,
            ...
            procurementInstructions[instructionIndex].spentBudget += quote.totalCost
            procurementInstructions[instructionIndex].acquiredCount += quote.count
            procurementInstructions[instructionIndex].lastResult = "業者間で\(quote.vehicleName)\(quote.count)台・\(quote.totalCost.currency)"
            recordProcurementActivity(
                instructionID: instructionID,
                source: .dealerTrade,
                acquiredCount: quote.count,
            ...
            updateEmployeePerformance(employeeID: handler.id, storeIndex: storeIndex) {
                $0.handled += 1
                $0.successes += 1
                $0.grossProfit += candidate.predictedUnitGrossProfit * quote.count
            }
            awardEmployeeExperience(employeeID: handler.id, storeIndex: storeIndex, focus: .procurement, successful: true)
            notes.append("\(stores[storeIndex].name)仕入指示：業者間で\(quote.vehicleName)\(quote.count)台を手配")
            return true

        case .auction(let listingID, let maxPrice):
            guard let listing = auctionListings.first(where: { $0.id == listingID }) else { return false }
            let committed = maxPrice + listing.venue.fee + listing.venue.shippingCost
            guard committed <= procurementInstructions[instructionIndex].remainingBudget,
                  reserveBid(listingID: listingID, storeID: storeID, maxPrice: maxPrice, instructionID: instructionID) else { return false }
            ...
            updateEmployeePerformance(employeeID: handler.id, storeIndex: storeIndex) { $0.handled += 1 }
            awardEmployeeExperience(employeeID: handler.id, storeIndex: storeIndex, focus: .procurement, successful: false)
            notes.append("\(stores[storeIndex].name)仕入指示：AAで\(listing.vehicleName)へ入札予約")
            return true

        case .online(let listingID, let maxPrice):
            guard let listing = onlineListings.first(where: { $0.id == listingID }) else { return false }
            let committed = maxPrice + listing.fee + listing.shippingCost
            guard committed <= procurementInstructions[instructionIndex].remainingBudget,
                  reserveOnlineBid(listingID: listingID, storeID: storeID, maxPrice: maxPrice, instructionID: instructionID) else { return false }
            procurementInstructions[instructionIndex].reservedBudget += committed
            ...
            updateEmployeePerformance(employeeID: handler.id, storeIndex: storeIndex) { $0.handled += 1 }
            awardEmployeeExperience(employeeID: handler.id, storeIndex: storeIndex, focus: .procurement, successful: false)
            notes.append("\(stores[storeIndex].name)仕入指示：ネットで\(listing.vehicleName)へ入札予約")
            return true
        }
    }
    ...
    private func applyDelegatedOperations(notes: inout [String]) {
        for index in stores.indices {
            ...
            if stores[index].delegateProcurement {
                ...
                var sources: Set<ProcurementSource> = [.auction, .dealerTrade, .corporateLot]
                if appraisalReady {
                    sources.formUnion([.storePurchase, .tradeIn, .online])
                }
            ...
            if !wonCompetition {
                ...
                if cash < playerTotal {
                    ...
                    inboundShipments.append(InboundShipment(
                        ...
                        monthsRemaining: listing.venue.shippingMonths
                    ))
                    simulationTransactionHandler?(SimulationVehicleTransaction(
                        turn: resolvingTurn,
                        kind: .acquired,
    ...
    private func resolveOnlineBids(at resolvingTurn: Int, notes: inout [String]) {
        let reservations = onlineBidReservations.filter { $0.resultTurn <= resolvingTurn }
        for bid in reservations {
            guard let listing = onlineListings.first(where: { $0.id == bid.listingID }) else {
                settleInstructionReservation(
                    instructionID: bid.instructionID,
                ...
                onlineBidReservations.removeAll { $0.id == bid.id }
                continue
            }
            let committedCost = bid.committedCost
            guard stores.contains(where: { $0.id == bid.storeID }) else {
            ...
            let rivalBid = competitorOnlineBid(for: listing, seed: seed)
            let rivalPrice = rivalBid?.maxPrice ?? listing.reservePrice
            let wonCompetition = bid.maxPrice >= rivalPrice
            let status: AuctionBidResultStatus
            var hammerPrice: Int
            ...
            if !wonCompetition {
                ...
                if cash < playerTotal {
                    ...
                    inboundShipments.append(InboundShipment(
                        ...
                        condition: listing.condition,
                        fault: listing.fault,
                        faultRevealed: onlineAssessment(
                            for: listing,
                            storeID: bid.storeID
                        ).detectedFault != nil,
                        instructionID: bid.instructionID,
                    ...
                    notes.append("全国ネット市場で\(listing.vehicleName)を\(hammerPrice.currency)で落札しました（諸費用込み \(playerTotal.currency)）")
                }
            }
            ...
            onlineBidResults.insert(OnlineBidResult(
                id: UUID(),
                listingID: listing.id,
                storeID: bid.storeID,
                modelID: listing.modelID,
            ...
            onlineListings.removeAll { $0.id == listing.id }
            onlineBidReservations.removeAll { $0.id == bid.id }
        }
        if onlineBidResults.count > 20 {
            onlineBidResults.removeLast(onlineBidResults.count - 20)
        }
    }
    ...
    private func competitorOnlineBid(for listing: OnlineListing, seed: Int) -> (competitorIndex: Int, maxPrice: Int)? {
        let bids = competitors.indices.compactMap { competitorIndex -> (competitorIndex: Int, maxPrice: Int)? in
            let competitor = competitors[competitorIndex]
    ...
    private func generateAuctionListings() {
        ...
        while auctionListings.count < 30 {
            ...
            let vehicleState = sourcingVehicleState(quality: profile.quality, seed: index * 197 + turn * 131, faultRate: 0.14)
            let faultDiscount = sourcingFaultDiscount(vehicleState.fault)
            let market = max(18, Int(Double(max(35, premiumImportFloor, calculatedMarket)) * faultDiscount))
    ...
    private func generateOnlineListings() {
        ...
        while onlineListings.count < 30 {
            ...
            let category = VehicleCategory.allCases[(index * 5 + turn) % VehicleCategory.allCases.count]
            let model = vehicleModel(for: category, seed: index * 73 + turn * 149)
            let profile = usedVehicleProfile(
                for: model,
            ...
            let market = max(12, Int(Double(calculatedMarket) * sourcingFaultDiscount(vehicleState.fault)))
            let reserve = max(10, market * (70 + index % 17) / 100)
            let premiumShipping = category == .imported || category == .pickup
    ...
    private func nicheCategories(for kind: MarketProductKind) -> [VehicleCategory] {
        ...
        case .sportTuned:
            return [.sports]
        case .welfare:
            return [.kei, .compact, .minivan, .commercial]
        case .mobileShop:
            return [.kei, .minivan, .commercial]
        case .standard:
            return VehicleCategory.allCases
        }
    }
    ...
    private func referralModelPool(for productKind: MarketProductKind, store: Store) -> [VehicleCatalogEntry] {
        ...
        case .welfare: available.filter { [.kei, .compact, .minivan, .commercial].contains($0.category) && !$0.isRareClassic }
        case .mobileShop: available.filter { [.kei, .minivan, .commercial].contains($0.category) && !$0.isRareClassic }
        case .camper: available.filter { $0.category == .minivan && !$0.isRareClassic }
        case .workCargo: available.filter { [.minivan, .commercial, .pickup].contains($0.category) && !$0.isRareClassic }
    ...
    private func generateSpecialtyReferralPurchaseCases() {
        for storeIndex in stores.indices where stores[storeIndex].isOperational {
            ...
            if productKind == .collector {
                ...
                guard rareRoll < 0.16 else { continue }
            }
            ...
            let desiredCount = min(5, max(1, Int((readiness * 4 + trendLift).rounded(.down))))
            var added = 0
            var attempt = 0
    ...
    private func customizationOrderTerms(
        ...
        let materialCost = max(20, Int(Double(model.referenceRetailPrice) * rateAndWork.0))
        ...
        let readiness = specialtyReadiness(for: store, productKind: productKind)
        let trend = segmentWillingnessFactor(for: key, store: store, productState: state)
        let revenue = max(
            materialCost + 35,
    ...
    private func generateCustomerCustomizationOrders() {
        for store in stores where store.isOperational
            ...
            && store.employees.contains(where: { $0.assignment == .service }) {
            guard let plot = plot(id: store.plotID),
                  let productKind = specialtyReferralProduct(for: store),
                  productKind != .collector else { continue }
            let readiness = specialtyReadiness(for: store, productKind: productKind)
            guard readiness >= 0.50 else { continue }
            let pool = referralModelPool(for: productKind, store: store)
            guard !pool.isEmpty else { continue }
            ...
            let count = min(3, whole + extra)
            for offset in 0..<count {
                let seed = turn * 23_123 + store.plotID * 431 + offset * 97 + simulationSeed
                let model = pool[abs(seed) % pool.count]
                let kind = referralProjectKind(for: productKind, seed: seed)
                let terms = customizationOrderTerms(kind: kind, model: model, store: store, district: plot.district)
    ...
    private func leadCategory(in kind: DistrictKind, seed: Int) -> VehicleCategory {
        ...
        let weighted = VehicleCategory.allCases.map { category -> (VehicleCategory, Double) in
            ...
            switch category {
            case .imported, .suv, .sports: economyMultiplier = 0.72 + economicIndex * 0.28
    ...
    func sellerCategory(in kind: DistrictKind, seed: Int) -> VehicleCategory {
        ...
        let weighted = VehicleCategory.allCases.map { category in
            ...
            case .sports: ownershipTurnover = 0.64
            }
            let highIncomeTurnover = [.suv, .imported, .sports].contains(category)
    ...
    private func makePurchaseCase(
        ...
        if let suggestedProjectKind,
           ...
           let targetState = suggestedProjectKind.productState {
            ...
            let ceiling = Int(Double(model.referenceRetailPrice) * priceCeilingFactor)
            let trendProgress = min(1, max(0, activeTrendMultiplier(for: key) - 1) / 2.2)
                * specialtyReadiness(for: store, productKind: productKind)
            let specialtyReferenceValue = Int(Double(model.referenceRetailPrice)
    ...
    static func makeDistricts() -> [District] {
        [
            District(kind: .downtown, population: 92_000, incomeIndex: 1.42, trafficIndex: 1.35, growthRate: 1.01, competition: 1.35, demands: [.imported: 1.58, .sports: 1.25, .suv: 1.18, .compact: 1.0, .kei: 0.72], supplies: [.compact: 0.95, .imported: 0.78, .sports: 0.85, .suv: 0.75, .kei: 0.55]),
            District(kind: .station, population: 76_000, incomeIndex: 1.03, trafficIndex: 1.42, growthRate: 1.015, competition: 1.28, demands: [.compact: 1.42, .kei: 1.25, .minivan: 1.08, .imported: 0.78, .sports: 0.65], supplies: [.compact: 1.45, .kei: 1.30, .minivan: 0.90, .imported: 0.55, .sports: 0.55]),
            District(kind: .emerging, population: 58_000, incomeIndex: 1.16, trafficIndex: 1.02, growthRate: 1.065, competition: 0.72, demands: [.suv: 1.52, .minivan: 1.45, .sports: 1.10, .compact: 0.95, .pickup: 0.88], supplies: [.suv: 1.12, .minivan: 1.05, .compact: 0.82, .sports: 0.75, .pickup: 0.60]),
            District(kind: .suburb, population: 88_000, incomeIndex: 1.08, trafficIndex: 1.18, growthRate: 1.02, competition: 1.02, demands: [.minivan: 1.48, .kei: 1.30, .suv: 1.26, .compact: 1.05, .sports: 0.90, .pickup: 0.82], supplies: [.minivan: 1.45, .kei: 1.35, .suv: 1.15, .compact: 1.0, .sports: 0.80, .pickup: 0.65]),
            District(kind: .industrial, population: 43_000, incomeIndex: 0.82, trafficIndex: 0.88, growthRate: 0.99, competition: 0.58, demands: [.commercial: 1.55, .pickup: 1.42, .kei: 1.15, .sports: 0.55, .imported: 0.32], supplies: [.commercial: 1.75, .pickup: 1.38, .kei: 0.92, .suv: 0.80, .sports: 0.45, .imported: 0.20]),
            District(kind: .highway, population: 66_000, incomeIndex: 0.91, trafficIndex: 1.48, growthRate: 1.012, competition: 0.93, demands: [.kei: 1.42, .sports: 1.35, .pickup: 1.34, .suv: 1.12, .commercial: 1.08], supplies: [.pickup: 1.25, .commercial: 1.22, .suv: 1.18, .kei: 1.05, .sports: 1.0, .minivan: 0.80])
        ]
    }
    ...
    static func makeCompetitors() -> [Competitor] {
        [
            ...
            Competitor(id: UUID(uuidString: "20000000-0000-0000-0000-000000000002")!, name: "プレミアモータース", strategy: "輸入車と品質保証", colorHex: "7356A8", cash: 58_000, plotIDs: [], strength: 1.15, category: .imported),
