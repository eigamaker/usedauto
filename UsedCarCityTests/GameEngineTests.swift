...
final class GameEngineTests: XCTestCase {
    ...
    private func startPlayableGame(
        ...
        game.stores[0].pendingPurchaseNegotiations = 0
    ...
    func testProcurementEmployeeAutomaticallySourcesMatchingInstruction() {
        ...
        XCTAssertTrue(
            game.inboundShipments.contains { $0.storeID == storeID && $0.category == .suv }
                ...
                || game.networkAuctionBidReservations.contains { $0.storeID == storeID && $0.instructionID == instruction.id }
        )
    }
    ...
    func testAuctionAndOnlineShippingCostsAreAboutOneThirdOfPreviousRates() {
        XCTAssertEqual(AuctionLane.standard.shippingCost, 2)
        XCTAssertEqual(AuctionLane.logistics.shippingCost, 4)
        XCTAssertEqual(AuctionLane.premium.shippingCost, 6)
        XCTAssertEqual(AuctionLane.standard.listingPricingShippingAllowance, 5)
        XCTAssertEqual(AuctionLane.logistics.listingPricingShippingAllowance, 12)
        XCTAssertEqual(AuctionLane.premium.listingPricingShippingAllowance, 18)

        let game = GameEngine()
        game.resetGame()
        startPlayableGame(game)

        XCTAssertEqual(game.networkAuctionListings.count, 144)
        XCTAssertTrue(game.networkAuctionListings.allSatisfy { [4, 6].contains($0.shippingCost) })
    }
    ...
    func testCompetitorAuctionPurchasePaysAllCostsAndAddsActualInventory() throws {
        ...
        let competitorIndex = try XCTUnwrap(game.competitors.indices.first(where: {
            game.competitors[$0].cash > listing.marketPrice + listing.lane.fee + listing.lane.shippingCost
        ...
        XCTAssertEqual(
            competitor.cash,
            cashBefore - listing.marketPrice - listing.lane.fee - listing.lane.shippingCost
        )
        XCTAssertEqual(
    ...
    func testAuctionWinChanceStillReflectsCompetitionAfterAdvancingAWeek() {
        let game = GameEngine(persistenceEnabled: false)
        ...
        let minivanChances = game.auctionListings
            .filter {
                $0.category == .minivan
                    && $0.fault != .major
                    && $0.fault != .immobile
            }
            .map { game.auctionBidWinChance(for: $0, maxPrice: $0.reservePrice) }
        XCTAssertFalse(minivanChances.isEmpty)
        XCTAssertTrue(
            minivanChances.allSatisfy { $0 < 1 },
            "minivan chances: \(minivanChances)"
        )
        XCTAssertGreaterThan(Set(minivanChances.map { Int(($0 * 100).rounded()) }).count, 1)
    }
    ...
    func testCompetitorAuctionBidsNeverExceedTheirResaleProfitCeiling() {
        ...
        for competitor in game.competitors {
            ...
            XCTAssertLessThan(
                ceiling + listing.lane.fee + listing.lane.shippingCost,
                retail,
    ...
    func testProcurementSourcesExposePricePolicyOneHundredMarginGuides() {
        ...
        XCTAssertEqual(ProcurementSource.networkAuction.baselineGrossMarginLabel, "2〜10%中心")
    ...
    func testFiftyTwoWeekBalanceScenariosAcrossFixedSeeds() throws {
    ...
    func testRobustGeneralStoresRunFor156WeeksAcrossFixedSeeds() throws {
        guard ProcessInfo.processInfo.environment["RUN_LONG_TERM_TESTS"] == "1" else {
            throw XCTSkip("長期シミュレーションは RUN_LONG_TERM_TESTS=1 の明示指定時のみ実行します")
        }
    ...
    func testInventoryCanBeConsignedToAuction() {
        ...
        XCTAssertTrue(game.consignInventory(storeID: store.id, category: category, count: 1))
    ...
    func testEmployeeCandidatesRefreshEveryWeekAndCoverEachSpecialty() {
        let game = GameEngine()
        game.resetGame()
        ...
        let expectedSpecialties = Set(["販売専門", "仕入専門", "調査専門", "整備技術者"])
        XCTAssertEqual(Set(firstWeek.map(\.specialtyName)), expectedSpecialties)
        XCTAssertGreaterThanOrEqual(
            firstWeek.first(where: { $0.specialtyName == "仕入専門" })?.procurementSkill ?? 0,
            76
        )
        XCTAssertGreaterThanOrEqual(
            firstWeek.first(where: { $0.specialtyName == "調査専門" })?.researchSkill ?? 0,
            76
        )
        for candidate in firstWeek {
            let commercialSkills = [candidate.salesSkill, candidate.procurementSkill, candidate.researchSkill]
            XCTAssertFalse(candidate.serviceSkill >= 70 && (commercialSkills.max() ?? 0) >= 70)
            if candidate.specialtyName == "整備技術者" {
                XCTAssertLessThanOrEqual(commercialSkills.max() ?? 0, 50)
            } else {
                XCTAssertLessThanOrEqual(candidate.serviceSkill, 50)
            }
        }
        ...
        XCTAssertEqual(Set(secondWeek.map(\.specialtyName)), expectedSpecialties)
    }
    ...
    func testEliteAbilityDoublesAutomationCapacityAndCommandsHigherSalary() {
        ...
        XCTAssertEqual(
            game.employeeWeeklyCaseCapacity(for: low, assignment: .procurement),
            game.employeeWeeklyCaseCapacity(for: low, assignment: .sales)
        )
        XCTAssertEqual(
            game.employeeWeeklyCaseCapacity(for: elite, assignment: .procurement),
            game.employeeWeeklyCaseCapacity(for: elite, assignment: .sales)
        )
    ...
    func testAutomaticSalesCommissionUsesOnlyPositiveGrossProfitAndHitsPersonnelCost() {
        ...
        game.stores[0].autoSales = true
        game.stores[0].salesPolicy = .volume
        game.buyerLeads = (0..<8).map { offset in
    ...
    func testUnverifiedAssessmentUsesServiceSkillAndNotProcurementSkill() {
        ...
        XCTAssertEqual(game.appraisalConfidence(for: storeID, source: .networkAuction), 90)
    }
    ...
    func testVerifiedAssessmentIsExactRegardlessOfEmployeeSkill() {
        ...
        for source in [ProcurementSource.auction, .corporateLot] {
    ...
    func testOwnerHasIndependentSevenSaleAndProcurementOpportunitiesWithoutStaff() {
        let game = GameEngine()
        game.resetGame()
        startPlayableGame(game, plan: .discount)
        game.cash = 100_000
        ...
        XCTAssertEqual(game.stores[0].purchaseNegotiationsThisWeek, 2)
        ...
        XCTAssertEqual(game.stores[0].manualNegotiationsThisWeek, 7)
        XCTAssertEqual(game.stores[0].purchaseNegotiationsThisWeek, 2)
        XCTAssertEqual(game.remainingWeeklyOpportunities(storeID: storeID), 0)
        XCTAssertEqual(game.remainingWeeklySalesOpportunities(storeID: storeID), 0)
        XCTAssertEqual(game.remainingWeeklyProcurementOpportunities(storeID: storeID), 5)
    ...
    func testManualAuctionBidConsumesProcurementCapacityOnlyOnFirstBid() throws {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 141)
        startPlayableGame(game, simulationSeed: 141)
        let storeID = try XCTUnwrap(game.stores.first?.id)
        game.stores[0].inventory = []
        let listings = Array(game.auctionListings.prefix(8))
        XCTAssertEqual(listings.count, 8)

        for listing in listings.prefix(7) {
            XCTAssertTrue(game.reserveBid(
                listingID: listing.id,
                storeID: storeID,
                maxPrice: listing.reservePrice
            ))
        }
        XCTAssertEqual(game.remainingWeeklyProcurementOpportunities(storeID: storeID), 0)
        XCTAssertEqual(game.remainingWeeklySalesOpportunities(storeID: storeID), 7)

        let first = try XCTUnwrap(listings.first)
        XCTAssertTrue(game.reserveBid(
            listingID: first.id,
            storeID: storeID,
            maxPrice: first.reservePrice + game.auctionBidStep(for: first)
        ))
        XCTAssertEqual(game.stores[0].purchaseNegotiationsThisWeek, 7)
        XCTAssertFalse(game.reserveBid(
            listingID: listings[7].id,
            storeID: storeID,
            maxPrice: listings[7].reservePrice
        ))

        game.cancelBid(listingID: first.id)
        XCTAssertEqual(game.remainingWeeklyProcurementOpportunities(storeID: storeID), 0)
        game.advanceWeek()
        XCTAssertEqual(game.remainingWeeklyProcurementOpportunities(storeID: storeID), 7)
        XCTAssertEqual(game.remainingWeeklySalesOpportunities(storeID: storeID), 7)
    }
    ...
    func testAutomaticSalesNeverExceedAvailableInventory() {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 41)
        startPlayableGame(game, plan: .discount, simulationSeed: 41)
        let storeID = game.stores[0].id
        let original = game.stores[0].inventory[0]
        ...
        game.stores[0].autoSales = true
        game.stores[0].salesPolicy = .volume
        game.buyerLeads = (0..<12).map { _ in
            BuyerLead(
                id: UUID(), storeID: storeID,
                preference: .category(original.category),
                budget: 100_000, minimumQuality: 0.1,
                minimumModelYear: 1990, maximumMileage: 1_000_000,
                priceSensitivity: 0, generatedTurn: game.turn
            )
        }
    ...
    func testProcurementInstructionDoesNotExposeOriginFiltering() throws {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 17)
        startPlayableGame(game, simulationSeed: 17)
        let storeID = try XCTUnwrap(game.stores.first?.id)
    ...
    func testImportedMinivanAppearsInAuctionAndOnlineSupply() {
        ...
        XCTAssertTrue(game.networkAuctionListings.contains {
            $0.category == .minivan
                && VehicleCatalog.entry(id: $0.modelID)?.origin == .imported
        })
    }
    ...
    func testSportsProcurementExistsInAuctionAndNeverInCorporateLots() {
        ...
        XCTAssertTrue(game.auctionListings.contains {
            $0.category == .sports && $0.lane == .premium
        })
    ...
    func testVersion52SaveIsNotLoadedAfterVersion53ProcurementInstructionChange() {
        let defaults = UserDefaults.standard
        defaults.removeObject(forKey: "UsedCarCity.save.v53")
        defaults.set(Data([0x01, 0x02, 0x03]), forKey: "UsedCarCity.save.v52")
        defer {
    ...
    func testProcurementInstructionNormalizesModelAndSupportsLifecycle() {
        ...
        instruction.minimumGrossProfit = 60
        XCTAssertTrue(game.updateProcurementInstruction(instruction))
        XCTAssertEqual(game.procurementInstructions(for: storeID).first?.minimumGrossProfit, 60)
    ...
    func testOnlineManualBidRequiresProcurementEmployeeAndAutomationOff() {
        ...
        game.cash = 100_000
        let storeID = game.stores[0].id
        let listing = game.networkAuctionListings[0]

        game.stores[0].employees = []
        XCTAssertFalse(game.reserveNetworkAuctionBid(
            listingID: listing.id,
            storeID: storeID,
        ...
        XCTAssertTrue(game.reserveNetworkAuctionBid(
            listingID: listing.id,
            storeID: storeID,
            maxPrice: max(listing.reservePrice, listing.marketPrice)
        ))
        game.cancelNetworkAuctionBid(listingID: listing.id)
        XCTAssertFalse(game.networkAuctionBidReservations.contains { $0.listingID == listing.id })
    ...
    func testAutomaticOnlineBidSettlesAndReleasesInstructionBudgetInWeeklyProcessing() {
        let game = GameEngine()
        game.resetGame()
        startPlayableGame(game)
        game.cash = 100_000
        ...
        let listing = try! XCTUnwrap(game.networkAuctionListings.first { listing in
            guard listing.reservePrice + listing.fee + listing.shippingCost < listing.marketPrice,
        ...
        game.networkAuctionListings = [listing]
        let instructionID = try! XCTUnwrap(game.createProcurementInstruction(
            storeID: storeID,
            totalBudget: 10_000,
            minimumGrossProfit: 0,
            categories: [listing.category],
            modelIDs: [listing.modelID]
        ))

        game.advanceWeek()

        XCTAssertFalse(game.networkAuctionBidReservations.contains { $0.instructionID == instructionID })
        let reservedInstruction = game.procurementInstructions.first { $0.id == instructionID }!
        XCTAssertEqual(reservedInstruction.reservedBudget, 0)
        let reservationReport = try! XCTUnwrap(game.lastReport?.procurement.first {
            $0.instructionID == instructionID && $0.source == .networkAuction
        })
        XCTAssertGreaterThan(reservationReport.reserved, 0)
        XCTAssertTrue(reservationReport.result.contains("入札"))
        XCTAssertLessThanOrEqual(
    ...
    func testWeeklyReportNotesAreSeparatedIntoCategories() {
        XCTAssertEqual(
            WeeklyReportCategory.category(for: "中央店仕入指示：AAでコンパクトへ入札"),
            .procurement
        )
        XCTAssertEqual(
    ...
    func testNetworkAuctionExpandsToOneHundredFortyFourWithThreeQuarterFlowStock() {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 91)
        game.startNewGame(simulationSeed: 91)

        XCTAssertEqual(game.networkAuctionListings.count, 144)
        XCTAssertEqual(game.networkAuctionListings.filter { $0.kind == .flowStock }.count, 108)
        XCTAssertEqual(game.networkAuctionListings.filter { $0.kind == .opportunity }.count, 36)
        XCTAssertTrue(game.networkAuctionListings.filter { $0.kind == .flowStock }.allSatisfy {
            $0.fault != .major && $0.fault != .immobile
        })
        for listing in game.networkAuctionListings.filter({ $0.kind == .flowStock }) {
            ...
            let assessment = game.networkAuctionAssessment(for: listing, storeID: UUID())
            let profit = retail - listing.reservePrice - listing.fee - listing.shippingCost - assessment.estimatedRepairCost
            let margin = Double(profit) / Double(max(1, retail))
            XCTAssertGreaterThanOrEqual(margin, 0.015)
            XCTAssertLessThanOrEqual(margin, 0.105)
        }
    }

    func testBothAuctionMarketsReplaceEveryListingEachWeek() {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 93)
        game.startNewGame(simulationSeed: 93)
        let originalVenueIDs = Set(game.auctionListings.map(\.id))
        let originalNetworkIDs = Set(game.networkAuctionListings.map(\.id))

        game.advanceWeek()

        let retainedVenue = originalVenueIDs.intersection(game.auctionListings.map(\.id))
        let retainedNetwork = originalNetworkIDs.intersection(game.networkAuctionListings.map(\.id))
        XCTAssertEqual(game.auctionListings.count, 72)
        XCTAssertEqual(game.networkAuctionListings.count, 144)
        XCTAssertTrue(retainedVenue.isEmpty)
        XCTAssertTrue(retainedNetwork.isEmpty)
    }

    func testCompetitorAuctionPlanUsesAtMostTwoSlotsPerBranchAndTargetsSixtyPercentStock() {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 94)
        game.startNewGame(simulationSeed: 94)
        let plannedSlots = Dictionary(uniqueKeysWithValues: game.competitors.map { competitor in
            ...
            return (competitor.id, slots)
        })

        game.advanceWeek()

        for competitor in game.competitors {
            let purchases = game.competitorAuctionPurchases.filter {
                $0.competitorID == competitor.id && $0.purchasedTurn == game.turn
            }
            XCTAssertLessThanOrEqual(purchases.count, plannedSlots[competitor.id, default: 0])
        }
    }

    func testProcurementInstructionValidatesGrossProfitAndNormalizesModels() throws {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 95)
        startPlayableGame(game, simulationSeed: 95)
        let storeID = try XCTUnwrap(game.stores.first?.id)

        XCTAssertNil(game.createProcurementInstruction(
            storeID: storeID,
            totalBudget: 10_000,
            minimumGrossProfit: -1,
            categories: Set(VehicleCategory.allCases)
        ))
        let id = try XCTUnwrap(game.createProcurementInstruction(
            storeID: storeID,
            totalBudget: 10_000,
            minimumGrossProfit: 30,
            categories: [.suv],
        ...
        let instruction = try XCTUnwrap(game.procurementInstructions.first { $0.id == id })
        XCTAssertEqual(instruction.minimumGrossProfit, 30)
        XCTAssertEqual(instruction.categories, [.suv])
        XCTAssertTrue(instruction.modelIDs.isEmpty)
    }

    func testProcurementInstructionUsesEmployeeWeeklyCapacity() throws {
        let game = GameEngine(persistenceEnabled: false)
        game.resetGame(simulationSeed: 97)
        startPlayableGame(game, simulationSeed: 97)
        game.cash = 100_000
        ...
        let instruction = try XCTUnwrap(game.procurementInstructions.first { $0.id == id })
        let reservations = game.bidReservations.filter { $0.instructionID == id }.count
            + game.networkAuctionBidReservations.filter { $0.instructionID == id }.count
        XCTAssertLessThanOrEqual(reservations, 12)
        XCTAssertGreaterThanOrEqual(instruction.reservedBudget, 0)
    }
